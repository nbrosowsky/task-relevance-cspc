...
```{r figure1, echo=FALSE, fig.cap="(ref:figure1)", fig.width=6.7,fig.height=4.51}
...
```{r figure2, echo=FALSE, fig.cap="(ref:figure2)", fig.width=6.875, fig.height= 4}
figure2
```
...
Similarly, the corresponding error analysis also resulted in a significant two-way interaction between the task-relevant context and unbiased-item congruency, `r RT_ANOVA$Task_Relevant_Context_Congruency`, and a non-significant three-way interaction between task-relevant context, unbiased-item congruency, and context-type, `r RT_ANOVA$Condition_Task_Relevant_Context_Congruency`. Therefore, the congruency effects in error rates were also smaller when the context dimension associated with high conflict was made task-relevant, corroborating the results of the reaction time analysis.
