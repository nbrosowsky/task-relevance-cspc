...
<!-- 
Furthermore, understanding how associations are formed and used is not well understood. Theories of automaticity and attentional control historically have emphasized the role of practice - difficult to suppress once learned.. Shiffrin and Schneider, Treisman, etc.. everyone basically.

Early CSPC work too thought that practice is important for "associations to form"

Memory-based theories on the other hand, take a more nuanced view. Practice is only important to the extent that improves memory retrieval. If retrieval cues can --  distinguish between prior episodes.

Therefore, the skill-learning view of automaticity would predict that associations take time to develop through practice, and once learned, difficult to suppress. 
-->
...
Previous work has shown that the effects of shifting between list-wide proportion congruent phases is asymmetrical. Abrahamse et al.... found that...  In our study, we were primarily interested in whether (a) participants could use context cues despite the presence of competing, contradictory cues, and (b) whether participants could switch from one context cue to another. We therefore randomized the order across participants to control for the effects of order. However, given that we did find systematic shifts in the CSPC effect for unbiased items, we can also ask whether the shift between phases was symmetrical in our study. 
...
Consistent with the original analysis, two-way interaction between task-relevant context and congruency remained significant,`r E1_Order_A$Congruency_Task_Relevant_Context`. However, the three-way interaction was non-significant, `r E1_Order_A$Order_Congruency_Task_Relevant_Context`. Therefor, we did not find statistical support for a list-shift asymmetry. Additionally, the asymmetry we do see in this sample is in the opposite direction than would be predicted given the prior results ().
...
Again, the three-way interaction was non-significant, `r E1_Order_B$Order_Congruency_Task_Relevant_Context`. Interestingly, the two-way interaction between congruency and task-relevant context is also still significant, `r E1_Order_B$Congruency_Task_Relevant_Context`. This result supports the notion there was no asymmetry and that the shift in context-dependent effects occur almost immediately after the transition.
...
<!-- ```{r echo=FALSE, echo = FALSE, warning = FALSE, message=FALSE, results='asis', out.width="\\textwidth"} -->
<!-- #print latex table -->
<!-- cat(TR_Table) -->
<!-- ``` -->
...
The results of the RT analysis revealed the critical, two-way interaction between proportion congruency and congruency to be non-significant, `r RT_ANOVA_E2$PC_Congruency`. Similarly, the corresponding error analysis also resulted in a non-significant two-way interaction between the task-relevant context and unbiased-item congruency, `r ACC_ANOVA_E2$PC_Congruency`. Therefore, there was no evidence that the context cues, absent the counting task, systematically influenced the congruency effect.
...
The results of the RT analysis revealed the critical, two-way interaction between task-relevant context and unbiased-item congruency to be non-significant, `r RT_ANOVA_E3$Task_Relevant_Context_Congruency`. Similarly, the corresponding error analysis also resulted in a non-significant two-way interaction between the task-relevant context and unbiased-item congruency, `r ACC_ANOVA_E3$Task_Relevant_Context_Congruency`.

# Estimating reproducability by simulation-based sensitivity- and power-analyses

```{r figure5, echo=FALSE, fig.cap='(ref:figure5)', fig.width=6.875, fig.height= 5}
#print figure 3
#source: data-analysis/plot-power-simulation.R
figure5
```

(ref:figure5) Results from Experiment 3 showing congruency effects (incongruent - congruent) for frequency unbiased items in reaction times (left) and error rates (right) as a function of the task-relevant context (100% proportion congruent versus 0% proportion congruent). Task-relevance was manipulated through explicit instructions.
